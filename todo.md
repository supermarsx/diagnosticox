...
- [ ] Patient list + overview UI: org/site filters, status chips, problem summaries; tie into patients/facts APIs.
 - [ ] Logging & tracing base: structured JSON logger (pino) implemented; correlation IDs / structured request logging middleware added; OpenTelemetry tracing wiring: basic console-backed tracing implemented (backend + optional frontend). Note: console.* replaced across backend seeds, migrations, RBAC and security controller. Next: wire OTLP exporter and CI/production config.

...
export async function seedSecurityData(db: IDatabase) {
  ...
  const departments = [
    { id: uuidv4(), name: 'Emergency Medicine', manager_id: userIds[2], budget: 3500000, staff_count: 42, location: 'Building A, Floor 1' },
    { id: uuidv4(), name: 'Internal Medicine', manager_id: userIds[3], budget: 2800000, staff_count: 35, location: 'Building B, Floor 2' },
    { id: uuidv4(), name: 'Cardiology', manager_id: userIds[4], budget: 3200000, staff_count: 28, location: 'Building B, Floor 3' },
    { id: uuidv4(), name: 'Neurology', manager_id: userIds[2], budget: 2900000, staff_count: 31, location: 'Building C, Floor 2' },
    { id: uuidv4(), name: 'Pediatrics', manager_id: userIds[3], budget: 2600000, staff_count: 38, location: 'Building A, Floor 3' },
    { id: uuidv4(), name: 'Radiology', manager_id: userIds[4], budget: 4100000, staff_count: 25, location: 'Building D, Floor 1' },
    { id: uuidv4(), name: 'Laboratory', manager_id: userIds[2], budget: 2200000, staff_count: 29, location: 'Building D, Floor 2' },
    { id: uuidv4(), name: 'Surgery', manager_id: userIds[3], budget: 3800000, staff_count: 33, location: 'Building C, Floor 4' },
  ];
  ...
  const authMethods = [
    ...
    { id: uuidv4(), user_id: userIds[1], method_type: 'email', method_value: 'dr.smith@clinic.com', is_primary: 1, verified_at: new Date().toISOString(), last_used_at: new Date().toISOString() },
    { id: uuidv4(), user_id: userIds[2], method_type: 'totp', method_value: 'KBSWY3DPEHPK3PXQ', is_primary: 1, verified_at: new Date().toISOString(), last_used_at: new Date(Date.now() - 3600000).toISOString() },
    { id: uuidv4(), user_id: userIds[3], method_type: 'biometric', method_value: 'fingerprint', is_primary: 1, verified_at: new Date().toISOString(), last_used_at: new Date().toISOString() },
  ];
  ...
  const auditLogs = [
    ...
    { id: uuidv4(), user_id: userIds[1], action: 'UPDATE', resource_type: 'patient', resource_id: 'patient-123', ip_address: '192.168.1.105', user_agent: 'Mozilla/5.0', status: 'success', severity: 'medium', details: '{"fields":["diagnosis"]}', timestamp: new Date(Date.now() - 3600000 * 4).toISOString() },
    { id: uuidv4(), user_id: userIds[2], action: 'ACCESS', resource_type: 'medical_record', resource_id: 'record-456', ip_address: '192.168.1.110', user_agent: 'Mozilla/5.0', status: 'success', severity: 'medium', details: '{"accessed_fields":["history","medications"]}', timestamp: new Date(Date.now() - 3600000 * 6).toISOString() },
    { id: uuidv4(), user_id: userIds[3], action: 'DELETE', resource_type: 'patient', resource_id: 'patient-789', ip_address: '192.168.1.115', user_agent: 'Mozilla/5.0', status: 'failure', severity: 'high', details: '{"error":"insufficient_permissions"}', timestamp: new Date(Date.now() - 3600000 * 8).toISOString() },
    ...
    { id: uuidv4(), user_id: userIds[4], action: 'LOGIN', resource_type: 'auth', resource_id: null, ip_address: '10.0.0.50', user_agent: 'Mozilla/5.0', status: 'failure', severity: 'critical', details: '{"reason":"invalid_credentials","attempts":3}', timestamp: new Date(Date.now() - 3600000 * 24).toISOString() },
    { id: uuidv4(), user_id: userIds[1], action: 'CREATE', resource_type: 'prescription', resource_id: 'rx-001', ip_address: '192.168.1.105', user_agent: 'Mozilla/5.0', status: 'success', severity: 'medium', details: '{"medication":"Lisinopril","dosage":"10mg"}', timestamp: new Date(Date.now() - 3600000 * 16).toISOString() },
    { id: uuidv4(), user_id: userIds[2], action: 'UPDATE', resource_type: 'security_settings', resource_id: 'policy-001', ip_address: '192.168.1.110', user_agent: 'Mozilla/5.0', status: 'success', severity: 'high', details: '{"changed":"password_policy"}', timestamp: new Date(Date.now() - 3600000 * 20).toISOString() },
    { id: uuidv4(), user_id: userIds[5], action: 'ACCESS', resource_type: 'medical_record', resource_id: 'record-999', ip_address: '192.168.1.120', user_agent: 'Mozilla/5.0', status: 'failure', severity: 'critical', details: '{"error":"access_denied","reason":"insufficient_clearance"}', timestamp: new Date(Date.now() - 3600000 * 30).toISOString() },
  ...
  const consents = [
    ...
    { id: uuidv4(), patient_id: patientIds[1], consent_type: 'marketing', purpose: 'Health and wellness communications', granted_by: userIds[1], granted_at: new Date(Date.now() - 86400000 * 60).toISOString(), expires_at: new Date(Date.now() + 86400000 * 305).toISOString(), status: 'active', scope: '{"channels":["email","sms"]}' },
    { id: uuidv4(), patient_id: patientIds[2], consent_type: 'third_party', purpose: 'Insurance claim processing', granted_by: userIds[0], granted_at: new Date(Date.now() - 86400000 * 15).toISOString(), expires_at: new Date(Date.now() + 86400000 * 350).toISOString(), status: 'active', scope: '{"partners":["insurance_provider_A"]}' },
    { id: uuidv4(), patient_id: patientIds[3], consent_type: 'research', purpose: 'Clinical research participation', granted_by: userIds[2], granted_at: new Date(Date.now() - 86400000 * 90).toISOString(), expires_at: new Date(Date.now() + 86400000 * 275).toISOString(), status: 'active', scope: '{"studies":["diabetes_study_2024"]}' },
    { id: uuidv4(), patient_id: patientIds[4], consent_type: 'data_sharing', purpose: 'Emergency care access', granted_by: userIds[1], granted_at: new Date(Date.now() - 86400000 * 180).toISOString(), expires_at: null, status: 'active', scope: '{"categories":["all"]}' },
  ];

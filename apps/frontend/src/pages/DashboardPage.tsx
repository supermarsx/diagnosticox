              ...
              filteredPatients.map((patient) => (
                ...
                >
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-3">
                      {(() => {
                        const s = patient.status || 'active';
                        const cls = s === 'active' ? 'bg-green-100 text-green-800' : s === 'stable' ? 'bg-blue-100 text-blue-800' : 'bg-yellow-100 text-yellow-800';
                        const label = s === 'active' ? 'Active' : s === 'stable' ? 'Stable' : 'Needs review';
                        return <span className={`px-4 py-2 rounded-full text-xs font-semibold ${cls}`}>{label}</span>;
                      })()}

...
async function seedSecurityDataOnly() {
  ...
  try {
    ...
    const departments = [
      ...
      { id: uuidv4(), organization_id: organizationId, name: 'Cardiology', manager_id: userIds[4] || userIds[0], budget: 3200000, user_count: 28, location: 'Building B, Floor 3' },
      { id: uuidv4(), organization_id: organizationId, name: 'Neurology', manager_id: userIds[2] || userIds[0], budget: 2900000, user_count: 31, location: 'Building C, Floor 2' },
      { id: uuidv4(), organization_id: organizationId, name: 'Pediatrics', manager_id: userIds[3] || userIds[0], budget: 2600000, user_count: 38, location: 'Building A, Floor 3' },
      { id: uuidv4(), organization_id: organizationId, name: 'Radiology', manager_id: userIds[4] || userIds[0], budget: 4100000, user_count: 25, location: 'Building D, Floor 1' },
      { id: uuidv4(), organization_id: organizationId, name: 'Laboratory', manager_id: userIds[2] || userIds[0], budget: 2200000, user_count: 29, location: 'Building D, Floor 2' },
      { id: uuidv4(), organization_id: organizationId, name: 'Surgery', manager_id: userIds[3] || userIds[0], budget: 3800000, user_count: 33, location: 'Building C, Floor 4' },
    ];

    for (const dept of departments) {
      await db.execute(
        `INSERT INTO departments (id, organization_id, name, manager_id, budget, user_count, location) 
         VALUES (?, ?, ?, ?, ?, ?, ?)`,
        [dept.id, dept.organization_id, dept.name, dept.manager_id, dept.budget, dept.user_count, dept.location]
      );
    }
    ...
    const authMethods = [
      ...
      { id: uuidv4(), user_id: userIds[1] || userIds[0], method_type: 'email', identifier: 'dr.smith@clinic.com', enabled: 1, verified: 1, last_used: new Date().toISOString(), metadata: '{}' },
      { id: uuidv4(), user_id: userIds[2] || userIds[0], method_type: 'totp', identifier: 'KBSWY3DPEHPK3PXQ', enabled: 1, verified: 1, last_used: new Date(Date.now() - 3600000).toISOString(), metadata: '{"device":"Tablet"}' },
      { id: uuidv4(), user_id: userIds[3] || userIds[0], method_type: 'biometric', identifier: 'fingerprint', enabled: 1, verified: 1, last_used: new Date().toISOString(), metadata: '{"type":"TouchID"}' },
    ];

    for (const method of authMethods) {
      await db.execute(
        `INSERT INTO auth_methods (id, user_id, method_type, identifier, enabled, verified, last_used, metadata) 
         VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
        [method.id, method.user_id, method.method_type, method.identifier, method.enabled, method.verified, method.last_used, method.metadata]
      );
    }

...
async function seedSecurityDataMinimal() {
  ...
  try {
    ...
    for (const [name, budget, count, location] of depts) {
      await db.execute(
        'INSERT INTO departments (id, organization_id, name, manager_id, budget, user_count, location) VALUES (?, ?, ?, ?, ?, ?, ?)',
        [uuidv4(), organizationId, name, userIds[0], budget, count, location]
      );
    }
    ...
    for (const [type, identifier, enabled, verified, metadata] of authMethods) {
      await db.execute(
        'INSERT INTO auth_methods (id, user_id, method_type, identifier, enabled, verified, last_used, metadata) VALUES (?, ?, ?, ?, ?, ?, ?, ?)',
        [uuidv4(), userIds[0], type, identifier, enabled, verified, new Date().toISOString(), metadata]
      );
    }
    ...
    for (const [action, table_name, record_id, changes, ip] of auditLogs) {
      await db.execute(
        'INSERT INTO audit_logs (id, organization_id, user_id, action, table_name, record_id, changes, ip_address) VALUES (?, ?, ?, ?, ?, ?, ?, ?)',
        [uuidv4(), organizationId, userIds[0], action, table_name, record_id, changes, ip]
      );
    }
    ...
    for (const [name, algo, size, purpose, status, rotDays, expDays] of keys) {
      ...
      await db.execute(
        'INSERT INTO encryption_keys (id, name, algorithm, key_size, purpose, status, last_rotation, expiry_date, usage_count) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)',
        [
          uuidv4(), name, algo, size, purpose, status,
    ...
    if (patients.length > 0) {
      ...
      for (let i = 0; i < Math.min(consents.length, patients.length); i++) {
        const patient = patients[i] as any;
        ...
        await db.execute(
          'INSERT INTO consent_records (id, patient_id, patient_name, consent_type, status, granted_date, expiry_date, scope) VALUES (?, ?, ?, ?, ?, ?, ?, ?)',
          [
            uuidv4(), patient.id, `${patient.first_name} ${patient.last_name}`,
            type, status,
